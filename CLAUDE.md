...
**Wizard defaults from play-area size (v760).** Both wizards (`SetupPage` = first-time/new-game at `/setup`; `GameSetupDialog` = edit-settings) auto-default game size AND allowed transit from the play area, until the user overrides either by hand (tracked by `sizeManuallySet` / `transitManuallySet`, both init `true` in edit mode so a saved game isn't clobbered). Shared, exported helpers in `GameSetupDialog.tsx`: `sizeForAreaKm2(km2)` (the rulebook S/M/L bands), `estimateTotalAreaKm2(primary, adjacents)` (sums the primary's bbox×`BBOX_FILL_FACTOR` estimate with EVERY added `additionalMapGeoLocations` entry — so adjacents ARE counted toward the suggested size, the previous gap), and `inferTransitModes(size)`: **Small = bus+tram, Medium = tram+subway+train, Large = tram+subway+train+ferry** (bus dropped for M/L — too slow/local past a walkable metro core; walking always implicit). Transit follows the EFFECTIVE size (a manual size choice still re-defaults the unmodified transit set, so bumping to Large pulls in ferry), so the auto-infer effect depends on `draftSize` with guarded/functional setters (`sameModes`) to avoid a size→transit loop.
...
build stamp. Current: `v752`. Use `git log` for the per-version detail;
