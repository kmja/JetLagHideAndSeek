...
- **`rail-station`** (`["railway"="station"]`) backs `same-train-line` matching, whose **seeker-side elimination is real** (v625): `determineMatchingBoundary` fetches all stations, finds the seeker's nearest, gets its line-mates via `trainLineNodeFinder` (same call the hider grades with), and unions the Voronoi cells of all same-line stations — so the map cut agrees with the hider's yes/no. (`same-first-letter-station` / `same-length-station` are still hider-graded only, `return false`.)
...
build stamp. Current: `v625`. Use `git log` for the per-version detail;
