...
## Endgame trigger (claim → hider confirm/refute, v618)

Per rulebook p43 the endgame begins when the seekers are physically inside the hider's **actual** zone and off transit; the hider then locks to a final hiding spot and can't move. The app models this as a **claim the hider can refute**, because seekers might go to the **wrong** station:

- **Seeker declares** via `seekerStartEndgame()` (`multiplayer/store.ts`) — from the `StationTransitCard` "Start endgame here" action (`allowEndgame`, seeker surface only; tap a zone on the map) OR the `HiderTimer` "Trigger endgame" button. Both stamp `endgameStartedAt` locally and send `{t:"startEndgame", at}`. Server (`GameRoom.handleStartEndgame`) idempotently stamps `setup.endgameStartedAt`, broadcasts `setupChanged`, **and Web-Pushes the offline hide team** (`pushEndgameToOfflineHideTeam`, mirrors the curse push) so a backgrounded hider on a train still gets the lock-down signal.
- **Hider refutes** a wrong claim via `hiderCancelEndgame()` → `{t:"cancelEndgame"}` (hide-team only). The `HiderHome` endgame banner has a **"They're not in my zone"** button (confirm dialog) for this. Server (`handleCancelEndgame`) resets `endgameStartedAt` to null + re-broadcasts; the seeker client detects the number→null mid-round transition (in the `setupChanged` handler, gated on `hidingPeriodEndsAt !== null` so new-round resets don't trip it) and notifies the seekers "the hider says you haven't reached their zone yet." Demo mode handles both messages in `demoBroker.ts`.
- After a confirmed endgame the hider commits a final spot via the existing `commitSpot` flow in `HiderHome`'s seeking view (phase flips to `endgame` once `hidingSpot` is set); Move powerup stays blocked while `endgameStartedAt !== null` (`roundActions.playMovePowerup`).

`endgameStartedAt` lives in `SetupState` (`protocol/state.ts`) so it rides the welcome snapshot for late joiners, and is a persistent atom (`gameSetup.ts`) so a reload mid-endgame keeps the state.
...
build stamp. Current: `v618`. Use `git log` for the per-version detail;
