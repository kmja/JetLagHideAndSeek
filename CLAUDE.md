...
2. **Broad fallbacks**: `navitia.ts` (Europe — France/Paris/Benelux/Iberia/Italy, free key), then `transitous.ts` — the **near-universal FREE + keyless** provider (MOTIS engine routing over the **Mobility Database** GTFS feeds, community-hosted at transitous.org). `canServe` = everywhere; outside its coverage it returns no itinerary and the dispatcher falls through to walking.
3. `walking.ts` — unconditional haversine×circuity backstop, so a journey is *always* produced.

**Cost constraint: every provider is genuinely free** — keyless or free-key-no-billing (Trafiklab, Entur, Digitransit, TfL, Swiss, Germany, Denmark, NSW, navitia, Transitous). Paid/billing-required providers (Google Directions, HERE) were tried and **removed** — they require a credit card and bill per request. Do NOT re-add a provider that needs billing.
