...
name-tag join) was correct in principle but the loaded tile PACK strips water
`name` tags (only `kind` survives), so on-device it dropped every inland body and
only the master-URL fallback restored names — too slow/fragile to be worth the
rulebook nicety. `getDissolvedBasemapWater` (all water) feeds the elimination
