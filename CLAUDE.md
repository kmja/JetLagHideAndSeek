...
- **Departures** (`POST /api/journey/departures`, `overpass-cache/src/departures/`, v644): a live stationboard — "what leaves THIS stop next?" — the hider reads to adapt on the fly (tap a zone → `StationTransitCard` → "Next departures" list). Mirrors the Plan dispatcher's **regional-first → MOTIS-fallback** model (`dispatcher.ts`, reusing the trip planner's `canServe` boxes), but the adapter set is a **subset** (not every planner backend exposes a board): shipped adapters are `trafiklab.ts` (SE ResRobot `location.nearbystops` → `departureBoard`, keyed) and `transitous.ts` (MOTIS `reverse-geocode` → `stoptimes`, universal keyless fallback, reused for a self-hosted box too). Each does a **two-step coord→board** (resolve nearest stop id, then fetch its board). Adding a region = one `fetchBoard` above MOTIS, exactly like the Plan tier grew one `plan` at a time. Same R2 + edge-cache pattern but SHORTER TTLs (2-min `when` bucket, 5-min R2, 2-min edge) since departures are time-sensitive. Wire types duplicated per side (`departures/types.ts` ↔ `src/lib/journey/departures.ts`). Parsers fixture-tested in `tests/departures.test.ts` (10 cases; MOTIS shapes modeled from the v2 API, degrade to an empty board on mismatch).
...
**Warm-city star (v642).** The play-area search (`PlayAreaStep` in `GameSetupDialog.tsx`) stars results that are **prewarmed** cities so users can spot fast, Overpass-free regions. The warm set comes from a public `GET /api/warm-cities` (worker `handleWarmCities` → curated/discovered cities with a backfilled `extent`, i.e. reference+adjacency-prewarmable), fetched once + cached in the `warmCityIds` atom (`src/maps/api/warmCities.ts`, `ensureWarmCitiesLoaded`/`isWarmCity`). It's a "which cities do we prewarm" hint, not live cache state — CDN/browser-cached 1 h. The operator's laptop-prewarm `--cold-only` (v641) warms the un-starred tail.
...
build stamp. Current: `v644`. Use `git log` for the per-version detail;
