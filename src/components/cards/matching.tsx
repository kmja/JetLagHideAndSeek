...
export const MatchingQuestionComponent = ({
    ...
    switch (data.type) {
        case "zone":
        case "letter-zone":
            // v1207: the admin level is a SEPARATE question chosen in the
            // subtype picker (each admin tier is its own tile) and named in
            // the card header, so the configure card no longer offers an admin-
            // division dropdown. Only the letter-zone simplification warning
            // remains.
            questionSpecific =
    ...
    return (
        ...
        >
            ...
            />
            {/* v611: subtype dropdown removed from the configure dialog —
                the subtype is chosen in the picker step and named in the
                header + nearest-reference box, so the dropdown was
                redundant clutter. */}
            {questionSpecific}
