 ...
 * (`CurseRevealOverlay`). Set to the curse card when a curse is cast on the
 * SEEKERS; the overlay plays the star-spin-in → card-spin-out → settle
 * sequence, then the seeker dismisses it (or it auto-dismisses). Cleared to
 * null when dismissed. Not persisted — a reveal is a one-shot moment, never
 * replayed on reload/reconnect (the curse itself lives in `receivedCurses`).
 */
export const curseReveal = atom<CurseCard | null>(null);
...
export function triggerCurseReveal(c: ReceivedCurse): void {
    curseReveal.set(curseCardFromReceived(c));
}
